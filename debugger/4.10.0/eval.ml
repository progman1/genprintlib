...
let rec expression event env ?orig_env = function
  | E_ident lid -> begin
      (* find the stack position of the ident but get the type from the regenerated env  *)
      match (match orig_env with
          | None -> env, Env.find_value_by_name lid env
          | Some orig_env->orig_env, Env.find_value_by_name lid orig_env) with
      | orig_env, (p, valdesc) ->
          let v =
            ...
            | Val_ivar (_, cl_num) ->
                let (p0, _) =
                  Env.find_value_by_name
                    (Longident.Lident ("self-" ^ cl_num)) orig_env
                in
                let v = value_path event orig_env p0 in
                let i = value_path event orig_env p in
                Debugcom.Remote_value.field v (Debugcom.Remote_value.obj i)
            | _ ->
                value_path event orig_env p
          in
          let (_, valdesc) = Env.find_value_by_name lid env in
  ...
  | E_item(arg, n) ->
      let (v, ty) = expression event env ?orig_env arg in

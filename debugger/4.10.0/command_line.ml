...
let print_expr depth ev env ppf expr =
  try
    match ev with
    | Some {ev_ev=ev'} ->
       (* Printf.printf "POS: %d\n" (Events.get_pos ev').pos_cnum; *)
       let gp_env=Genprint.debug_on_module ev'.ev_loc ev'.ev_module in
       let (v, ty) = Eval.expression ev gp_env ~orig_env:env expr in
       print_named_value depth expr gp_env v ppf ty
    | None ->

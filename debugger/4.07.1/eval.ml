...
let rec expression event env ?orig_env = function
    E_ident lid ->
      begin try
        let orig_env, (p, valdesc) =
          (* find the stack position of the ident but get the type from the regenerated env  *)
          match orig_env with
          | None -> env, Env.lookup_value lid env
          | Some orig_env-> orig_env, Env.lookup_value lid orig_env in
        (begin match valdesc.val_kind with
           Val_ivar (_, cl_num) ->
             let (p0, _) =
               Env.lookup_value (Longident.Lident ("self-" ^ cl_num)) orig_env
             in
         ...
         let (_, valdesc) = Env.lookup_value lid env in
